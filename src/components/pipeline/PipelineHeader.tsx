...
export function PipelineHeader() {
  ...
  const [newMonthlyOffer, setNewMonthlyOffer] = useState("");

  // Summary stats
  const stats = useMemo(() => {
    const total = properties.length;

    const signedAndLive = properties.filter(
      (p) => p.stage === "signed" || p.stage === "live"
    );
    const totalMonthlyRent = signedAndLive.reduce(
      (sum, p) => sum + (p.monthly_offer || 0),
      0
    );

    const pitched = properties.filter((p) => p.stage === "pitched").length;
    const signedFromPitch = properties.filter(
      (p) => p.stage === "signed" || p.stage === "live" || p.stage === "negotiating"
    ).length;
    const conversionRate =
      pitched + signedFromPitch > 0
        ? Math.round((signedFromPitch / (pitched + signedFromPitch)) * 100)
        : 0;

    return { total, totalMonthlyRent, conversionRate };
  }, [properties]);
  ...
  const handleAddProperty = () => {
    ...
    const newProperty: PipelineProperty = {
      ...
      user_id: "user-1",
      address: newAddress,
      landlord_name: newLandlordName || undefined,
      monthly_offer: newMonthlyOffer ? parseInt(newMonthlyOffer) : undefined,
      stage: stages[0]?.id || "identified",
      position: 0,
      labels: [],
      priority: null,
    ...
    toast({
      ...
      description: "Property has been added to your landlord pipeline.",
    });
  };
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted/50 border">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-primary">{formatCurrency(stats.totalMonthlyRent)}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted/50 border">
          <TrendingUp className="h-4 w-4 text-muted-foreground" />
          <div>
            <p className="text-xs text-muted-foreground">Pitch → Signed</p>
          ...
          <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Track a new landlord lead in your pipeline.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="offer">Monthly Offer £ (optional)</Label>
                  <Input
                    id="offer"
                    type="number"
                    placeholder="1200"
                    value={newMonthlyOffer}
                    onChange={(e) => setNewMonthlyOffer(e.target.value)}
                  />
                </div>
              </div>
              <DialogFooter>
